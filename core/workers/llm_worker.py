...
def _build_prompt(task: LLMTask, review_summary: dict) -> str:
    ...
    if task.exchange in _NO_REVIEW_EXCHANGES or rev_status == "NOT_SUPPORTED":
        ...
        lines.append(
            f"Reviews: API відгуків потребує автентифікації — тимчасово недоступний. Оцінюй за умовами та поведінкою. НЕ вважай відсутність відгуків фактором ризику.")
    elif rev_status == "NO_SESSION":
        # Браузерна сесія ще не перехоплена — тексти відгуків недоступні
        if bad_texts:
            lines.append(
                f"Reviews: Сесія ще не перехоплена, але є {len(bad_texts)} збережених поганих відгуків:")
        elif is_estimated and total > 0:
            lines.append(
                f"Reviews (⚠️ ОЦІНКА з completion rate, сесія ще не перехоплена): ~pos≈{pos}, ~neg≈{neg}, ~neg%≈{neg_pct:.1f}%"
            )
            lines.append(
                "❌ ТЕКСТИ ВІДГУКІВ НЕДОСТУПНІ (браузерна сесія ще не перехоплена). "
                "Причини негативних відгуків НЕВІДОМІ. "
                "Вкажи це ЧЕСНО у thought_process. НЕ вигадуй аналіз відгуків!"
            )
        else:
            lines.append(
        ...
        lines.append(
            f"Reviews (⚠️ ОЦІНКА з completion rate, НЕ реальні відгуки): ~pos≈{pos}, ~neg≈{neg}, ~neg%≈{neg_pct:.1f}%"
        )
        if neg == 0:
            lines.append("⬆️ Оцінка: жодного негативного (completion rate ≈100%).")
        elif neg_pct < 3.0:
            lines.append(f"⬆️ Оцінка: лише ~{neg} негативних (~{neg_pct:.1f}%). Але це ОЦІНКА — реальні причини невідомі.")
        else:
            lines.append(
                f"⚠️ ОЦІНКА показує ~{neg} негативних (~{neg_pct:.1f}%). Це може означати зірвані угоди, скарги, або технічні проблеми — ПРИЧИНИ НЕВІДОМІ."
            )
        if not bad_texts:
            lines.append(
                "❌ ТЕКСТИ ВІДГУКІВ НЕДОСТУПНІ (потрібна браузерна сесія). "
                "Ми НЕ ЗНАЄМО чому саме були негативні відгуки — можливо скам/шахрай, можливо просто повільний продавець. "
                "Вкажи це ЧЕСНО у thought_process: 'тексти відгуків недоступні, причини neg≈X невідомі'. "
                "НЕ вигадуй що ти проаналізував відгуки якщо текстів немає!"
            )
        ...
        if bad_texts:
            ...
            # Є числа але немає текстів — показати числа з поясненням
            lines.append(
                f"Reviews: API недоступний ({rev_status}). Відомо: pos={pos}, neg={neg}. "
                f"Але ТЕКСТИ відгуків відсутні — причини негативних НЕВІДОМІ."
            )
        else:
            lines.append(
                f"Reviews: API тимчасово недоступний ({rev_status}). Оцінюй за умовами та поведінкою. НЕ вважай це фактором ризику.")
    else:
        if total > 0:
    ...
    elif neg > 0 and not is_estimated:
        # Є негативні в статистиці але тексти не завантажені
        lines.append(
            f"⚠️ Є {neg} негативних відгуків, але їх ТЕКСТИ не завантажені. "
            f"Причини негативних відгуків НЕВІДОМІ. Вкажи це чесно у thought_process."
        )
