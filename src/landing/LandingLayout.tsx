  ...
  return (
    <div className="page-enter relative min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-accent-500/30">
      ...
      */}
      ...
      {/*
        Смужка прогресу читання живе ПОЗА шапкою, і це не косметика.

        Усередині вона була дочірнім елементом липкого хедера, який
        браузер виносить в окремий композитний шар. Смуга в один піксель
        кольору акценту, яку ще й масштабують через scaleX, у такому шарі
        — рецепт для помилок растеризації: на мобільному Chrome шапка
        зеленіла на всю висоту саме при скролі, тобто рівно тоді, коли
        масштаб змінюється з нуля.

        Тут вона фіксована зверху, у власному шарі, і висота реальна —
        два пікселі замість одного. Субпіксельна смуга в композитному
        шарі округлюється по-різному на різних щільностях екрана.
      */}
      <div
        className="scroll-progress fixed top-0 left-0 z-50 h-0.5 w-full bg-accent-500"
        aria-hidden
      />
