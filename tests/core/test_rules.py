...
import pytest
from core.analysis.rules import HARD_RULES, SOFT_RULES, SAFE_RULES, WARN_RULES, ALL_RULES
...
def _first_hard(text: str):
    # Resolve active suppressors
    suppressed = set()
    for r in SAFE_RULES:
        if _matches(r, text):
            sh = getattr(r, "suppress_hard", None)
            if sh:
                suppressed.update(sh)

    for r in HARD_RULES:
        if r.category in suppressed:
            continue
        if _matches(r, text):
            return r
    return None
